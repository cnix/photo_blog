$(document).ready(function($) {
  ...
  if ( $('.photo').length ) {
    ...
    $.getJSON('http://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=' + api_key + '&photoset_id=' + set_365_id + '&extras=tags&format=json&jsoncallback=?',
      function (data) {
        ...
        image_container = "<a href='" + photo_page_url + "' target='_blank'>";
  ...
  if ( $('#index').length ) {
    $.getJSON('http://api.flickr.com/services/rest/?method=flickr.people.getPublicPhotos&api_key=' + api_key + '&user_id=' + user_id + '&per_page=5&page=1&format=json&jsoncallback=?',
      function(data) {
